...
def compute_v(
    ...
    var_subjects = ["Cristiano Ronaldo","Lionel Messi"]
    lookup_idxs_kl = [
        find_fact_lookup_idx(
            prompt, var_subjects[i], tok, hparams.fact_token, verbose=(i == 0)
        )
        for i, prompt in enumerate(kl_prompts)
    ]
    lookup_idxs = lookup_idxs_re + lookup_idxs_kl
